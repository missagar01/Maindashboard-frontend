...
import { Loader, BadgeCheck, Loader2, CheckCircle, User, Calendar, Hash, RefreshCcw } from "lucide-react";
...
export default function StoreGRNGMApproval() {
    ...
    const handleApproveGM = async (row: StoreGRNRow) => {
        ...
        try {
            ...
            toast.success(`GRN ${row.grn_no} approved by GM successfully`);
    ...
    const baseRows = useMemo(() => {
        // Only show those already sent & admin approved
        return rows.filter(r => r.sended_bill && r.approved_by_admin);
    }, [rows]);

    const pendingRowsRaw = useMemo(() => {
        return baseRows.filter(r => !r.approved_by_gm);
    }, [baseRows]);
    ...
    const renderAction = (row: StoreGRNRow) => {
        ...
        return (
            <Button
                ...
                className="w-full md:w-auto bg-purple-600 hover:bg-purple-700 text-white font-medium shadow-sm transition-all flex items-center justify-center gap-2"
            >
                {isProcessing ? (
                    <>
        ...
        <div className="w-full p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto">
            ...
            {loading ? (
                ...
                <div className="mx-auto max-w-lg text-center p-16 border-2 border-dashed rounded-3xl bg-gray-50/50 mt-4">
                    ...
                    <p className="text-gray-500 mb-6 px-4">There are no records matching your current filter.</p>
                    <Button
                        ...
                        <table className="w-full text-sm text-left border-collapse">
                            <thead className="bg-slate-50 border-b sticky top-0 z-10 shadow-sm">
                                <tr className="text-slate-600 font-semibold">
                                    ...
                                    <th className="px-5 py-4 text-center bg-slate-50">Status</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={row.grn_no} className="hover:bg-slate-50/80 transition-colors group">
                                        ...
                                        <td className="px-5 py-5">
                                            <div className="flex flex-col gap-1 items-center">
                                                <span className="text-[10px] uppercase font-black text-slate-400 tracking-tighter">Process Stage</span>
                                                <div className="flex gap-1.5">
                                                    ...
                                                    {row.approved_by_gm && (
                                                        <div title="GM Approved" className="w-6 h-6 rounded-lg bg-purple-100 flex items-center justify-center text-purple-600 shadow-sm border border-purple-200">
                                                            <BadgeCheck size={14} />
                                                        </div>
                                                    )}
                                                </div>
