...
import { createMaterialConsumption, listTasks } from '../../../api/project/projectApi';
...
const MaterialConsumptionModal = ({ isOpen, onClose, materials, onRefresh }) => {
  ...
  const inputClass =
    'w-full rounded-xl border border-slate-200 bg-white px-4 py-3.5 text-sm font-medium text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-red-300 focus:ring-4 focus:ring-red-100';
  const fieldLabelClass =
    'ml-1 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500';
  const primaryButtonClass =
    'inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-rose-500 to-red-600 px-5 py-3.5 text-[11px] font-black uppercase tracking-[0.18em] text-white shadow-[0_18px_30px_-18px_rgba(220,38,38,0.65)] transition hover:brightness-105 disabled:opacity-50 sm:w-auto sm:min-w-[220px]';
  ...
  const fetchActivities = async () => {
    try {
      ...
      setActivities(Array.isArray(data?.rows) ? data.rows : Array.isArray(data) ? data : []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createMaterialConsumption(formData);
      onRefresh();
      onClose();
      setFormData(getInitialFormData());
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/30 p-3 backdrop-blur-sm sm:flex sm:items-center sm:justify-center sm:p-4">
      <div className="mx-auto my-4 w-full max-w-xl overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-[0_30px_70px_rgba(15,23,42,0.18)] sm:my-8 sm:max-h-[90vh] sm:rounded-[2rem]">
        <div className="border-b border-slate-100 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(255,241,242,0.92))] px-4 py-4 sm:px-6 sm:py-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex min-w-0 items-center gap-4">
              <div className="rounded-[1.2rem] border border-red-100 bg-red-50 p-3 text-red-500 shadow-sm">
              <Package size={24} />
              </div>
              <div className="min-w-0">
                <p className="text-[10px] font-black uppercase tracking-[0.24em] text-red-500/80">
                  Material Registry
                </p>
                ...
                <p className="mt-2 text-sm font-medium leading-6 text-slate-500">
                  Record issued stock against work activity or general site consumption.
                </p>
              </div>
            </div>
            <button
