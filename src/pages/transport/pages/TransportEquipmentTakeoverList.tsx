...
import {
  fetchEquipmentTakeoverList,
  type EquipmentTakeoverRecord,
} from "../../../api/transport/equipmentTakeoverApi";
...
const getBranchName = (record: EquipmentTakeoverRecord) => {
  return safeText(
    record.branch_name ||
    record.branch?.name ||
    record.branch_code
...
const getShiftName = (record: EquipmentTakeoverRecord) => {
  return safeText(
...
const getEquipmentName = (record: EquipmentTakeoverRecord) => {
  return safeText(record.equipment_name || record.equipment?.name || record.equipment_code);
};

const getDriverName = (record: EquipmentTakeoverRecord) => {
...
const getHandoverCode = (record: EquipmentTakeoverRecord) => {
  return safeText(record.equipment_handover_code || record.handover_code || record.equipment_handover_id);
};

const searchableValues = (record: EquipmentTakeoverRecord) => [
  record.id,
...
const getLoadErrorMessage = (error: unknown) => {
  ...
  return "Equipment Takeover List load nahi ho paya.";
};

export default function TransportEquipmentTakeoverList() {
  const [records, setRecords] = useState<EquipmentTakeoverRecord[]>([]);
  ...
  const loadRecords = async (signal?: AbortSignal, showRefreshing = false) => {
    ...
    try {
      const response = await fetchEquipmentTakeoverList({}, signal);
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto max-w-[1600px] px-2 py-3 sm:px-5 sm:py-4">
            ...
            <div className="flex flex-col gap-2 xl:flex-row xl:items-end xl:justify-end">
              <div className="relative min-w-0 sm:w-[320px]">
                ...
                <input
                  ...
                  placeholder="Search equipment, driver, handover, branch..."
                  className="w-full rounded-2xl border border-slate-200 bg-white py-2.5 pl-10 pr-3 text-sm font-medium text-slate-700 outline-none transition focus:border-blue-400"
                />
              </div>
          ...
          {isFetching ? (
            <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-blue-100 bg-blue-50 px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.16em] text-blue-700">
              <RefreshCw className="h-3.5 w-3.5 animate-spin" />
              Fetching live takeover data...
            </div>
          ) : null}
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
            ...
            <div className="rounded-2xl border border-slate-200 bg-blue-50 px-3 py-3">
            ...
            <div className="rounded-2xl border border-slate-200 bg-emerald-50 px-3 py-3">
              ...
              <p className="mt-1 text-lg font-black text-slate-900 sm:text-2xl">
                {isFetching
                  ? "..."
                  : new Set(filteredRecords.map((record) => getHandoverCode(record))).size}
              </p>
            </div>
          </div>
        </div>
              ...
              <table className="w-full text-left table-fixed">
                <thead className="bg-slate-900 text-white">
                  <tr>
                    ...
                    <th className="w-[15%] px-4 py-3 text-[11px] font-black uppercase tracking-[0.16em]">Driver</th>
                    <th className="w-[15%] px-4 py-3 text-[11px] font-black uppercase tracking-[0.16em]">Closing KMR</th>
                    <th className="w-[15%] px-4 py-3 text-[11px] font-black uppercase tracking-[0.16em]">Closing HMR</th>
                    <th className="w-[20%] px-4 py-3 text-[11px] font-black uppercase tracking-[0.16em]">Linked Handover</th>
                    <th className="w-[10%] px-4 py-3 text-[11px] font-black uppercase tracking-[0.16em]">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {isFetching ? (
                    ...
                    <tr>
                      <td colSpan={6} className="px-4 py-16 text-center text-sm font-semibold text-slate-500">
                        Equipment takeover records nahi mile.
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((record, index) => (
                      <tr key={record.id || `${record.equipment_id}-${index}`} className="transition hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-sm font-bold text-slate-900 truncate">
                          {getHandoverCode(record)}
                        </td>
